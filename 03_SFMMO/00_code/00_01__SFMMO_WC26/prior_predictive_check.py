"""The prior-predictive check: what goal distribution do the priors imply before seeing data?

Rebuilds the prior structure on a synthetic standardized design and samples per-team goals.
Finding: the shipped team attack/defence scale priors (Gamma(2,4) times ZeroSumNormal(1)) let
exp(eta) explode (q99.9 ~ 100+ goals); a tightened ZeroSumNormal(0.30) team prior is sane.

Needs the Bayesian stack (uv sync --group model).
Run: uv run python 00_code/prior_predictive_check.py
"""
    ...
    with pm.Model(coords=coords) as m:
        ...
        if team_kind == "hier":  # shipped: Gamma scale x ZeroSumNormal(1)
            ...
            alpha = pm.Deterministic(
                "alpha",
                pm.ZeroSumNormal("alpha_raw", sigma=1, dims="teams") * sigma_alpha,
                dims="teams",
            )
            sigma_delta = pm.Gamma("sigma_delta", alpha=2, beta=team_scale)
            delta = pm.Deterministic(
                "delta",
                pm.ZeroSumNormal("delta_raw", sigma=1, dims="teams") * sigma_delta,
                dims="teams",
            )
        ...
        eta = (
            ...
            + kappa[il]
            + X_home * beta_home[it]
            + pm.math.dot(X_gf, beta)
        )
        pm.Poisson(
            "match_outcome", mu=pm.math.exp(eta), observed=np.zeros(n_obs, dtype=int), dims="obs_id"
        )
    return m
...
def summarize(tag, goals):
    ...
    print(
        "   P(>6)={:.4f}  P(>10)={:.5f}  P(>15)={:.6f}  P(>30)={:.7f}".format(
...
def main():
    ...
    ax.hist(
        ...
        density=True,
        alpha=0.55,
        label=f"shipped (max {g_ship.max()})",
        color="C3",
    )
    ax.hist(
        np.clip(g_fix, 0, 15),
        ...
        histtype="step",
        lw=2.2,
        label=f"proposed (max {g_fix.max()})",
        color="C0",
    )
    ...
    print(
        f"PROPOSED prior is sane: P(>15)={f_fix[15]:.4f}, P(>30)={f_fix[30]:.6f}, "
