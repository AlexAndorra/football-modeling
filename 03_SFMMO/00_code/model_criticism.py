"""model_criticism.py — Phase 2 of the Bayesian workflow: criticize the fitted model.

Loads the per-fold InferenceData saved by sfmmo_fit.py and runs the checks the first pass
skipped:

  - PRIOR SENSITIVITY (power-scaling, `arviz_stats.psense_summary`): are the conclusions robust
    to the ZeroSumNormal(0.30) team prior, or are parameters prior-driven? Needs log_prior +
    log_likelihood in the idata (sfmmo_fit now computes both).
  - IDENTIFIABILITY: posterior correlations among the global intercept (mu), the league effects
    (kappa) and mean home advantage (beta_home). Components that are near-collinear (|corr| ~ 1)
    are not separately identified — the technical review flagged league-vs-home-advantage.

(W/D/L forecast calibration via the randomized PIT is already covered in sfmmo_eval.py /
compare_improved.py — p ~ 0.97 for the calibrated model.)
...
def psense_family_summary(ps):
    ...
    for fam, grp in ps.groupby(base):
        ...
        rows.append(rec)
    summary = (pd.DataFrame(rows).sort_values("max_prior", ascending=False)
...
def main():
    ...
    md = ("# SFMMO improved model — criticism (prior sensitivity + identifiability)\n\n"
