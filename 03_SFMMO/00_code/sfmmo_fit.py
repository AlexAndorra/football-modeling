...
# =============================================================================
_HERE = os.path.dirname(os.path.abspath(__file__))           # .../03_SFMMO/00_code
_SFMMO_ROOT = os.path.dirname(_HERE)                          # .../03_SFMMO
...
dict_EW = {'trainT':[['WMQ2010','WM2010','WMQ2014','WM2014'],
...
if SMOKE:
    dict_EW = {'trainT': [dict_EW['trainT'][-1]],
               'valT':   [dict_EW['valT'][-1]]}
    print("\n[SMOKE] last fold only:", dict_EW['valT'][0][0],
          "(train through", dict_EW['trainT'][0][-1] + ")\n")


# ===== cell 7 =====
def build_confederation_map(data):
    """team -> confederation, inferred from the qualifier league(s) the team appears in."""
    conf_leagues = [l for l in data['name_league'].unique() if str(l).startswith('wm-qualifikation')]
      ...
      if ss_idx > 0:
        ...
        for team in ELO_rating.keys():
          if team in ss_t1__teams:
            ELO_rating[team] = ELO_rating[team] * (1 - regress_w) + regress_to * regress_w
          else:
...
def compute_global_elo(complete_data, K=20, home_adv=50, regress_w=0.0, regress_to=1500.0):
    ...
    for _, row in m.iterrows():
        ...
        if pd.notna(row['goalsscored_inGame_team']) and pd.notna(row['goalsscored_inGame_opp']):
            elo[h], elo[a] = update_elo(eh, ea, int(row['match_outcome__home']), K=K, home_adv=home_adv)
    ...
    for _, row in complete_data.iterrows():
        ...
        if row['home_pitch'] == 1:
            et.append(eh); eo.append(ea)        # home perspective
        else:
            et.append(ea); eo.append(eh)        # away perspective
...
def main():
    ...
    for t in range(len(dict_EW['trainT'])):
      ...
      confed_map = {key: [] for key in [f'wm-qualifikation-{c}' for c in ['asien', 'europa', 'afrika', 'nordamerika-mittelamerika', 'suedamerika', 'ozeanien']]}
      for c in confed_map.keys():
        confed_map[c] = data_raw.loc[data_raw['name_league'] == c,'name_team'].unique().tolist()
      ...
      # --- 'elo' weighting:
      if 1==1:
        complete_data['elo_team'] = np.where(complete_data['name_team'].isin(confed_map['wm-qualifikation-asien']),complete_data['elo_team'] * 0.8,complete_data['elo_team'])
        complete_data['elo_opp'] = np.where(complete_data['name_opp'].isin(confed_map['wm-qualifikation-asien']),complete_data['elo_opp'] * 0.8,complete_data['elo_opp'])

        complete_data['elo_team'] = np.where(complete_data['name_team'].isin(confed_map['wm-qualifikation-afrika']),complete_data['elo_team'] * 0.8,complete_data['elo_team'])
        complete_data['elo_opp'] = np.where(complete_data['name_opp'].isin(confed_map['wm-qualifikation-afrika']),complete_data['elo_opp'] * 0.8,complete_data['elo_opp'])

        complete_data['elo_team'] = np.where(complete_data['name_team'].isin(confed_map['wm-qualifikation-nordamerika-mittelamerika']),complete_data['elo_team'] * 0.75,complete_data['elo_team'])
        complete_data['elo_opp'] = np.where(complete_data['name_opp'].isin(confed_map['wm-qualifikation-nordamerika-mittelamerika']),complete_data['elo_opp'] * 0.75,complete_data['elo_opp'])

        complete_data['elo_team'] = np.where(complete_data['name_team'].isin(confed_map['wm-qualifikation-ozeanien']),complete_data['elo_team'] * 0.6,complete_data['elo_team'])
        complete_data['elo_opp'] = np.where(complete_data['name_opp'].isin(confed_map['wm-qualifikation-ozeanien']),complete_data['elo_opp'] * 0.6,complete_data['elo_opp'])
      ...
      if do__scaleCS:
        ...
        train_means = complete_data.groupby(['season','id_league','gameday'])[factors_CS].mean()
        ...
        data__scaleCS = complete_data.groupby(['season','id_league','gameday'])[factors_CS].apply(_cs_scale).reset_index().set_index('level_3').drop(['season','id_league','gameday'],axis=1)
      ...
      with SFMMO__dev:
          ...
          eta = pm.Deterministic('eta',
                                mu + alpha[idx_team] - delta[idx_opp] + kappa[idx_league]
                                + X_home * beta_home[idx_team] + pt.dot(X_gf, beta.T))
      ...
      if do__scaleCS:
        data__scaleCS = data_oos.groupby(['season','id_league','gameday'])[factors_CS].apply(_cs_scale).reset_index().set_index('level_3').drop(['season','id_league','gameday'],axis=1)
