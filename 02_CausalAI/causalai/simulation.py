...
def simulate_matches(
    ...
    # Individual Treatment Effects (ITE) = Y(1) - Y(0) for everyone
    ...
    lam_tackles = (0.10 * def_new + np.maximum(score_diff, 0)) * fresh_legs_boost
    tackles = rng.poisson(lam_tackles)

    # Clearances (fresh legs help defensive actions)
    lam_clear = (0.08 * def_new + 1.5 * np.maximum(score_diff, 0)) * fresh_legs_boost
    clearances = rng.poisson(lam_clear)

    # Blocks
    lam_blocks = (0.05 * def_new + np.maximum(score_diff, 0)) * fresh_legs_boost
    blocks = rng.poisson(lam_blocks)

    # Pressures (higher when trailing, boosted by fresh legs)
    lam_press = (0.12 * def_new + 1.2 * np.maximum(0, -score_diff)) * fresh_legs_boost
    pressures = rng.poisson(lam_press)

    # Dribbles/Carries (attacking actions benefit from substitutions)
    lam_drib = (0.06 * off_new + np.maximum(0, -score_diff)) * tactical_factor
    ...
    # Fouls (scale with pressures, fresh players might be more aggressive)
    lam_fouls = 0.04 * pressures * fresh_legs_boost + 0.5
